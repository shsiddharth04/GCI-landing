...
function LeadInstructor({ inst }: { inst: Instructor }) {
  return (
    <motion.div
      ...
      style={{ background: '#0f0d18', border: '1px solid rgba(212,191,255,0.12)', position: 'relative', overflow: 'hidden', marginBottom: '1px' }}
    >
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '1px', background: 'linear-gradient(90deg, transparent, rgba(212,191,255,0.6), transparent)', zIndex: 2 }} />
        ...
        <div className="lead-photo" style={{ position: 'absolute', inset: 0 }}>
          {inst.photoUrl ? (
            <>
              ...
              <div style={{ position: 'absolute', inset: 0, background: 'rgba(212,191,255,0.04)' }} />
            </>
          ) : (
            <div style={{ width: '100%', height: '100%', background: 'rgba(212,191,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ fontSize: '56px', fontWeight: 800, color: 'rgba(212,191,255,0.15)', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <div style={{ flex: 1, padding: 'clamp(24px, 4vw, 48px) clamp(20px, 4vw, 44px)', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '9px', color: '#d4bfff', letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: '28px' }}>
          {inst.role}
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '32px' }}>
            {inst.credentialLines.map((line, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <div style={{ width: '4px', height: '4px', borderRadius: '50%', background: '#d4bfff', flexShrink: 0 }} />
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            {inst.socialLinks.map((link, i) => (
              <a
                ...
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = '#d4bfff'; (e.currentTarget as HTMLElement).style.borderColor = 'rgba(212,191,255,0.4)' }}
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = 'rgba(212,191,255,0.5)'; (e.currentTarget as HTMLElement).style.borderColor = 'rgba(212,191,255,0.18)' }}
              >
...
function SecondaryInstructor({ inst }: { inst: Instructor }) {
  return (
    <motion.div
      variants={fadeUp}
      style={{ background: '#0f0d18', border: '1px solid rgba(212,191,255,0.1)', display: 'flex', gap: '20px', padding: '24px', position: 'relative', overflow: 'hidden' }}
    >
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '1px', background: 'linear-gradient(90deg, transparent, rgba(212,191,255,0.3), transparent)' }} />
      ...
      {inst.photoUrl ? (
        ...
        <div style={{ flexShrink: 0, width: '72px', height: '72px', background: 'rgba(212,191,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <span style={{ fontWeight: 700, color: '#d4bfff', fontSize: '16px', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{inst.initials || inst.name.slice(0, 2).toUpperCase()}</span>
        </div>
      )}

      <div>
        <div style={{ fontSize: '15px', fontWeight: 700, fontFamily: "'Plus Jakarta Sans', sans-serif", marginBottom: '4px' }}>{inst.name}</div>
        <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '9px', color: 'rgba(212,191,255,0.55)', letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: '10px' }}>{inst.role}</div>
...
function PlaceholderLead() {
  return (
    <motion.div
      initial="hidden" whileInView="visible" viewport={viewportOnce}
      variants={fadeUp}
      style={{ background: '#0f0d18', border: '1px solid rgba(212,191,255,0.1)', minHeight: '400px', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}
    >
      ...
      <div style={{ textAlign: 'center' }}>
        <div style={{ width: '56px', height: '56px', background: 'rgba(212,191,255,0.07)', margin: '0 auto 16px' }} />
      ...
      <div style={{ maxWidth: '1152px', margin: '0 auto' }}>
        ...
        >
          <p style={{
            ...
            color: '#050505', letterSpacing: '0.28em', textTransform: 'uppercase', marginBottom: '16px',
            background: '#d4bfff', padding: '5px 12px',
          }}>
            Taught by
          </p>
          <h2 style={{ fontSize: 'clamp(2.5rem, 5vw, 3.5rem)', fontWeight: 800, lineHeight: 1.1, letterSpacing: '-0.025em', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
            Practitioners.<br /><span style={{ color: '#d4bfff' }}>Not academics.</span>
          </h2>
        </motion.div>
          ...
          <>
            ...
            {secondary.length > 0 && (
              <motion.div
                ...
                style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '1px', marginTop: '1px', background: 'rgba(212,191,255,0.08)' }}
              >
