...
// Authenticity is verified via HMAC-SHA256 on the raw body using the course-specific secret.
// This secret is intentionally separate from RAZORPAY_WEBHOOK_SECRET (masterclass).

const RAZORPAY_COURSE_WEBHOOK_SECRET = Deno.env.get('RAZORPAY_COURSE_WEBHOOK_SECRET')!
...
async function verifySignature(rawBody: string, receivedSig: string): Promise<boolean> {
  ...
  const key = await crypto.subtle.importKey(
    'raw', enc.encode(RAZORPAY_COURSE_WEBHOOK_SECRET),
...
async function generateInvoicePdf(opts: {
  ...
  const invNum = `CE-${opts.enrollmentId.slice(0, 8).toUpperCase()}`
  ...
  page.drawText('DJ Course — Seat Deposit', { x: margin + 10, y, size: 11, font: fontBold, color: black })
  ...
  page.drawText('Gurugram studio · Batch to be confirmed', { x: margin + 10, y, size: 9, font, color: gray })
  ...
  const footerY = 52
  page.drawLine({
    start: { x: margin, y: footerY + 18 }, end: { x: right, y: footerY + 18 },
...
async function generateAndSendInvoice(
  ...
  if (!enrollment) {
    console.error('generateAndSendInvoice: enrollment not found', opts.enrollmentId)
    return
  }
  ...
  const fileName = `course-deposit-${opts.enrollmentId}.pdf`
  ...
  await fetch(`${SUPABASE_URL}/functions/v1/send-course-confirmation`, {
    method: 'POST',
    headers: {
...
serve(async (req) => {
  ...
  if (event.event === 'payment.captured') {
    ...
    if (paymentRec) {
      ...
      // Only flip enrollment status if it hasn't already been marked paid
      await supabase
        .from('course_enrollments')
